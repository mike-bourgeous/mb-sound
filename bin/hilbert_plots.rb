...
filters = {
  ...
  fine_scale: (1.5..2.5).step(0.05).map { |i| MB::Sound::Filter::HilbertIIR.new(scale: i) },
  very_fine_scale: (2.1..2.3).step(0.01).map { |i| MB::Sound::Filter::HilbertIIR.new(scale: i) },
  scale_and_stretch: (0.8..1.4).step(0.1).map { |i| MB::Sound::Filter::HilbertIIR.new(scale: 2.25, stretch: i) },
...
filters.each do |group, flist|
  ...
  flist.each.with_index do |f, idx|
    ...
    d = d[20..20000]
    puts "Filter at index #{idx}: #{MB::U.highlight(first: d[0], last: d[-1], min: d.min, max: d.max, mean: d.mean)}"
