...
voices = OSC_COUNT.times.map { |i|
  ...
  cenv = MB::Sound.adsr(0, 0.005, 0.5, 2.5, auto_release: false).db(30)
  cenv2 = MB::Sound.adsr(0, 0.01, 0.5, 2.5, auto_release: false).db(60)
  c = cenv * note2.dup.at(1).fm(cenv2 * note2.dup.at(1)).forever

  denv = MB::Sound.adsr(0, 0.005, 0.0, 2, auto_release: false).db(50)
  d = denv * MB::Sound::Tone.new(frequency: note2.dup.frequency.constant * 0.9996 - 0.22).at(1).forever

  eenv = MB::Sound.adsr(0, 2, 0, 4, auto_release: false).db
  e = eenv * note.dup.at(1).fm(c * 4810 + d * 500).forever
  fenv = MB::Sound.adsr(0, 2, 0, 4, auto_release: false).db
