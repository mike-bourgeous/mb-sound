#!/usr/bin/env ruby
# A root + fifth pad patch for sequences: two slightly detuned saws per
# interval, slow swells, a lowpass that opens with each chord, and a little
# Haas stereo width.  Returns a stereo pair of graph nodes.
#
...
module MB::Sound
  ...
  def self.fifth_pad(clip, cutoff: 1400, detune: 0.07, attack: 0.6, release: 2.5, width: 0.012)
    voice = ->(c) { c.tone.ramp.at(0.5) + c.transpose(detune).tone.ramp.at(0.5) }
    swell = clip.env(attack, 1.0, 0.8, release)
    bloom = clip.env(attack * 1.5, 2.0, 0.3, release)

    pad = ((voice.(clip) + voice.(clip.transpose(7)) * 0.7) * swell * 0.35)
