RSpec.describe(MB::Sound::FastWavetable) do
  ...
  describe '.outer_linear' do
    it 'can retrieve a value from a wavetable' do
      expect(MB::Sound::FastWavetable.outer_linear(Numo::SFloat[[0, 1], [-1, 2]], 0, 0)).to eq(0)
      expect(MB::Sound::FastWavetable.outer_linear(Numo::SFloat[[0, 1], [-1, 2]], 0.5, 0.5)).to eq(2)
      expect(MB::Sound::FastWavetable.outer_linear(Numo::SFloat[[0, 1], [-1, 2]], 0.5, 0)).to eq(-1)
    end
  end
