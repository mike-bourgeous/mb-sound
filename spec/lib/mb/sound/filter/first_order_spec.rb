RSpec.describe MB::Sound::Filter::FirstOrder do
  ...
  context 'lowpass' do
    it 'has a response of 0.707 at the cutoff frequency' do
      ...
      expect(Sound.sigfigs(f.response(2.0 * Math::PI * 2000 / 48000).abs, 6)).to eq(Sound.sigfigs(0.5 ** 0.5, 6))

      f = MB::Sound::Filter::FirstOrder.new(:lowpass, 48000, 6000)
      expect(Sound.sigfigs(f.response(2.0 * Math::PI * 6000 / 48000).abs, 6)).to eq(Sound.sigfigs(0.5 ** 0.5, 6))
    end
  end
