    ...
    [:csv, :tsv, :yml, :json].each do |ext|
      context "when reading from .#{ext}" do
        ...
        it 'can load a one-row matrix' do
          ...
          p = MB::Sound::MatrixProcess.from_file("tmp/matrix_test.#{ext}")
        ...
        it 'raises an error if the data contains non-numeric entries' do
          ...
          expect {
            MB::Sound::MatrixProcess.from_file("tmp/matrix_test.#{ext}")
          }.to raise_error(MB::Sound::MatrixProcess::MatrixTypeError)
        end
