RSpec.describe(MB::Sound::Wavetable, aggregate_failures: true) do
  ...
  pending '.load_wavetable'
  pending '.save_wavetable'

  describe '.wavetable_lookup' do
    it 'can perform lookups based on arrays' do
      number = Numo::SFloat[0, 1.0 / 3.0, 2.0 / 3.0, -2.0 / 3.0]
      phase = Numo::SFloat[0, 0.4, 0.6, 1.2]

      result = MB::Sound::Wavetable.wavetable_lookup(wavetable: table, number: number, phase: phase)
      expect(MB::M.round(result, 5)).to eq(MB::M.round(Numo::SFloat[1, 3, 0.1, -3], 5))
    end
  end
  ...
  describe '.outer_lookup' do
    ...
    it 'can interpolate between rows' do
      ...
      expect(MB::Sound::Wavetable.outer_lookup(wavetable: table, number: 3.0 / 6.0, phase: 0.6).round(6)).to eq(-0.45)
    end
